...
> Admin/process items are marked as N/A in the evidence column.
...
| REQ-28 | Must store Pergunta objects in object files (serialized). | [done] | Evidence: `QuestionCache#savePerguntasNormais`/`savePerguntasBonus` with `ObjectOutputStream`; `build-cache` creates `.ser` files including `data/cache/perguntas_bonus.ser` when bonus file exists. |
...
| REQ-31 | There is one file per money level (200/500/1000/3000/10000/50000) + one bonus file. | [done] | Evidence: normal files expected in repo root; bonus file `perguntas_bonus.txt` in repo root and discoverable via `QuestionPaths#findBonusTextFile`. |
